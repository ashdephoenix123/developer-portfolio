...
const BlogCard = ({ blog: { title, metadescription, author, authorimg, date, slug } }) => {
  ...
  return (
    <div className='block bg-gray-200 text-black rounded my-2 p-4 hover:bg-gray-300 dark:bg-gray-600 dark:text-zinc-300 scaleIt transition-all'>
      <h2 className='head3 font-semibold dark:text-neutral-300 flex justify-between items-center mb-2'>{title} 
      {!heart ? <div className='w-8 ml-4'><BsHeart onClick={changeHeart} className='hover:cursor-pointer max-[456px]:text-2xl' /> </div>: <div  className='w-8 ml-4'><BsHeartFill className='text-pink-600 hover:cursor-pointer max-[456px]:text-2xl heartAnimation' onClick={changeHeart} /></div>}
      ...
      <p className='fontsm min-[1200px]:text-base mt-1 mb-3'>{metadescription.slice(0,300).concat('...')}</p>
      <Link className='btn' href={`/blogs/${slug}`}>Read</Link>
    </div>
  )
}
