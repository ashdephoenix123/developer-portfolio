...
const Sidebar = () => {
    ...
    return (
        <div className='ml-4 w-1/2 border-2 bg-gray-50 border-gray-100 text-black rounded-lg p-4 self-start max-[880px]:ml-0 max-[880px]:mt-4 max-[880px]:w-full dark:bg-gray-600 dark:border-gray-600 dark:text-zinc-300'>
            ...
            <p className='flex items-center'>Give this article a Like &nbsp; {!heart ? <BsHeart onClick={changeHeart} className='hover:cursor-pointer' size={25}/> : <BsHeartFill className='text-pink-600 hover:cursor-pointer' onClick={changeHeart}  size={25}/>}</p>
        </div>
    )
}
